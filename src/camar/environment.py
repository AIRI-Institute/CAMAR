...
class Camar:
    def __init__(
        ...
        assert isinstance(map_generator, base_map), (
            "map_generator must be an instance of base_map"
        )

        self.dynamic = dynamic
        assert isinstance(dynamic, BaseDynamic), (
            "dynamic must be an instance of BaseDynamic"
        )
        ...
        self.agent_rad = map_generator.agent_rad  # will be changed
        self.goal_rad = map_generator.goal_rad

        if pos_shaping_factor is None:
            self.pos_shaping_factor = 0.024 / self.agent_rad
        else:
        ...
        if max_obs is not None:
            ...
            self.max_obs = int(self.window / self.landmark_rad) ** 2

        self.max_obs = min(self.max_obs, self.num_agents + self.num_landmarks - 1)
        ...
        self.observation_spaces = Box(
            -jnp.inf, jnp.inf, shape=(self.num_agents, self.observation_size)
        )
        ...
        # lifelong
        self.map_reset = (
            map_generator.reset_lifelong if lifelong else map_generator.reset
        )
        self.update_goals = (
            map_generator.update_goals
            if lifelong
            else lambda keys, goal_pos, to_update: (keys, goal_pos)
        )
    ...
    def reset(self, key: ArrayLike) -> tuple[Array, State]:
        ...
        state = State(
            ...
            goal_pos=goal_pos,
            is_collision=jnp.full(
                (self.num_agents,), False, dtype=jnp.bool_
            ),  # no checks on reset
            step=0,
            on_goal=on_goal,
    ...
    def step(
        ...
        old_goal_dist = jnp.linalg.norm(
            state.physical_state.agent_pos - state.goal_pos, axis=-1
        )  # (num_agents, )

        def frameskip(scan_state, _):
            key, physical_state, landmark_pos, actions, is_collision = scan_state

            key, _key = jax.random.split(key)
            physical_state, is_collision = self._world_step(
                _key, physical_state, landmark_pos, actions, is_collision
            )

            return (key, physical_state, landmark_pos, actions, is_collision), None

        is_collision = jnp.zeros(shape=(self.num_agents,), dtype=jnp.int32)

        (key, physical_state, landmark_pos, actions, is_collision), _ = jax.lax.scan(
            frameskip,
            init=(key_w, state.physical_state, state.landmark_pos, actions, is_collision),
            xs=None,
        ...
        goal_dist = jnp.linalg.norm(
            physical_state.agent_pos - state.goal_pos, axis=-1
        )  # (num_agents, )
        on_goal = goal_dist < self.goal_rad
        ...
        goal_keys, goal_pos = self.update_goals(
            state.goal_keys, state.goal_pos, on_goal
        )

        just_arrived = jnp.logical_not(state.on_goal) & on_goal
        current_time = (state.step + 1) * self.step_dt
        time_to_reach_goal = jnp.where(
            just_arrived, current_time, state.time_to_reach_goal
        )
    ...
    def get_obs(self, state: State) -> Array:
        ...
        nearest_dists, nearest_ids = jax.lax.top_k(
            -dists, self.max_obs + 1
        )  # (num_agents, self.max_obs + 1)
        ...
        nearest_rad = jnp.where(
            nearest_ids < self.num_agents, self.agent_rad, self.landmark_rad
        )  # (num_agents, self.max_obs)

        obs_dists_coeff = (
            self.agent_rad + nearest_rad
        ...
        obs_coeff = (
            self.window + nearest_rad
        ) / nearest_dists  # (num_agents, self.max_obs)
        obs = (
            nearest_ego_objects * (1.0 - obs_coeff)[:, :, None]
        )  # (num_agents, self.max_obs, 2)
        ...
        ego_goal_norm = jnp.where(
            goal_dist[:, None] > 1.0, ego_goal / goal_dist[:, None], ego_goal
        )
    ...
    def get_reward(
        self, is_collision: ArrayLike, goal_dist: ArrayLike, old_goal_dist: ArrayLike
    ) -> Array:
        on_goal = goal_dist < self.goal_rad

        r = (
            0.5 * on_goal.astype(jnp.float32)
            - 1.0 * is_collision.astype(jnp.float32)
            + self.pos_shaping_factor * (old_goal_dist - goal_dist)
        )
    ...
    def _world_step(
        ...
        collision_force, is_collision = self._apply_environment_force(
            collision_force, is_collision, physical_state, landmark_pos
        )

        # integrate state
        physical_state = self.dynamic.integrate(
            key, collision_force, physical_state, actions
        )

        return physical_state, is_collision

    def _apply_environment_force(
        self,
        ...
        agent_forces, is_collision_agents = self._get_collision_force(
            physical_state.agent_pos[agent_idx_i],
            physical_state.agent_pos[agent_idx_j],
            self.agent_rad + self.agent_rad,
        ...
        landmark_forces, is_collision_landmarks = self._get_collision_force(
            physical_state.agent_pos[agent_idx],
            landmark_pos[landmark_idx],
            self.agent_rad + self.landmark_rad,
    ...
    def _get_collision_force(
        self, pos_a: ArrayLike, pos_b: ArrayLike, min_dist: float
    ) -> tuple[Array, Array]:
